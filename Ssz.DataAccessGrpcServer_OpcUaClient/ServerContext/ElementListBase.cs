...
namespace Ssz.DataAccessGrpcServer_OpcUaClient
{
        ...
        where TElementListItem : ElementListItemBase
    {
        ...
        /// <returns></returns>
        public override async Task<List<AddItemToListResult>> AddItemsToListAsync(List<ListItemInfo> itemsToAdd)
        {
            if (Disposed) throw new ObjectDisposedException("Cannot access a disposed DataListRoot.");

            if (itemsToAdd.Count == 0) 
                return new List<AddItemToListResult>();

            var results = new List<AddItemToListResult>(itemsToAdd.Count);
                    ...
                    results.Add(new AddItemToListResult
                        {
                            ...
                            DataTypeId = new Ssz.DataAccessGrpc.ServerBase.TypeId()
                        }
                        );
                    continue;
                }
            ...
            foreach (AddItemToListResult r in results)
            {
                if ((StatusCode)r.AliasResult.StatusCode != StatusCode.OK)
                {
                    TElementListItem? item;
                    if (ListItemsManager.TryGetValue(r.AliasResult.ServerAlias, out item))
                    {
        ...
        protected virtual Task<List<AddItemToListResult>> OnAddElementListItemsToListAsync(List<TElementListItem> elementListItems)
        {
