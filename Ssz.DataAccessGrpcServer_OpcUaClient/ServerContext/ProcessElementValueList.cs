...
namespace Ssz.DataAccessGrpcServer_OpcUaClient
{
    ...
    public class ProcessElementValueList : ElementValueListBase<ProcessElementValueListItem>
    {
            ...
            if (nowUtc >= LastCallbackTime.AddMilliseconds(UpdateRateMs))
            {
                ...
                if (device is null)
                {
                    foreach (ProcessElementValueListItem item in ListItemsManager)
                    {
                        ...
                        item.UpdateValueStatusTimestamp(new ValueStatusTimestamp());
                    }
                }
                else
                {
        ...
        protected override Task<List<AddItemToListResult>> OnAddElementListItemsToListAsync(List<ProcessElementValueListItem> elementListItems)
        {
