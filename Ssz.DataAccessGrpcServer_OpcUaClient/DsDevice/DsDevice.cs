...
using Cdt.Dcs.CentralServer.Common;
...
namespace Ssz.DataAccessGrpcServer_OpcUaClient
{
    public partial class DsDevice: IDisposable
    {
                ...
                using (var writer = new SerializationWriter(memoryStream, true))
                {
                    ...
                    {                        
                        ...
                        {                            
                            ...
                            {
                                writer.WriteOwnedData(module, context);
                            }
                        }
                    }
                }
                                ...
                                foreach (int i in Enumerable.Range(0, count))
                                {
                                    ...
                                    using (reader.EnterBlock())
                                    {
                                        if (module is not null)
                                            reader.ReadOwnedData(module, context);
                                    }
                                }
                                break;
                            default:
        ...
        {            
            ...
            {
                ElementIdsMap.Initialize(CsvDb.GetValues(ElementIdsMap.StandardMapFileName), CsvDb.GetValues(ElementIdsMap.StandardTagsFileName), CsvDb);
